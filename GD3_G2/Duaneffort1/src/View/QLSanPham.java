...
//        FillSanPham();
    ...
    public boolean chk() {
        ...
        if (ValiDate.isNull(txtSanPham, "Không được để trống Mã sản phẩm")) {
            return false;
        } else if (ValiDate.isNull(txtTenSP, "Không được để trống Tên ")) {
            return false;
        } else if (ValiDate.isNull(txtGiaBan, "Không được để trống giá bán")) {
            return false;
        } else if (ValiDate.isInt(txtGiaBan, " Giá bán phải là số ")) {
            return false;
        } else if (ValiDate.isNull(txtGiaNhap, " Giá nhập không được để trống ")) {
            return false;
        } else if (ValiDate.isInt(txtGiaNhap, "Giá nhập phải là số ")) {
            return false;
        } else if (txtSanPham.getText().matches(p_ten) == false) {

            JOptionPane.showMessageDialog(this, "Mã loại sản phẩm không đúng định dạng");
            return false;
        } else if (Float.valueOf(txtGiaBan.getText()) <= 0) {
            JOptionPane.showMessageDialog(this, " số lượng ko đc nhỏ hơn 0");
            return false;
        } else if (Float.valueOf(txtGiaNhap.getText()) <= 0) {
            JOptionPane.showMessageDialog(this, " số lượng ko đc nhỏ hơn 0");
            return false;
        } else {
            return true;
    ...
    public boolean chk2() {
        if (ValiDate.isNull(txtSoLuong, "Không được để trống số lượng")) {
            return false;
        } else if (ValiDate.isInt(txtSoLuong, "Số Lượng phải số")) {
            return false;
        }
        if (Float.valueOf(txtSoLuong.getText()) <= 0) {
            JOptionPane.showMessageDialog(this, " số lượng ko đc nhỏ hơn 0");
            return false;
        } else {
            return true;
